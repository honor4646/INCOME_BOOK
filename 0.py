...
class App(tk.Tk):
    def __init__(self):
        ...
        self.result0.set("- Расположение итогового xlsx файла КУД под именем: год-месяц-дата-времяКУД year.xlsx")
    ...
    def pattern(self):
        ...
        filename = fd.askopenfilename(title='Исходный xlsx файл в котором оформляется КУД',
                                      initialdir='/home/oleg/Yandex.Disk/0YandexDisk/PYTHON_ubuntu/Repositories/'
                                                 'INCOME_BOOK/PATTERN', filetypes=filetypes)
    ...
    def in_file(self):
        ...
        filename = fd.askopenfilename(title='xlsx файл для загрузки данных ОФД',
                                      initialdir='/home/oleg/Yandex.Disk/0YandexDisk/PYTHON_ubuntu/Repositories/'
                                                 'INCOME_BOOK/DOWNLOADS', filetypes=filetypes)
    ...
    def result(self):
        ...
        result = f'/home/oleg/Yandex.Disk/0YandexDisk/PYTHON_ubuntu' \
                 f'/Repositories/INCOME_BOOK/RESULT/КУД{self.year_ent.get()}_{dt.now().year}-{dt.now().month}-' \
...
# [counter, дата из date_time_col, nomer_fd_col, ’Парикмахерские услуги’, ofd_income_col]
        ...
        for row in range(header_row + 1, last_row+1):
            ...
            matrix_row = [counter, doc_date,
                          ...
                          str(data_sheet.cell(row, ofd_income_col).value)]
